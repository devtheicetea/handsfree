...
import { startRepl, type Repl } from "./repl.js";
...
const conn = connect({
  ...
  onStatus: (s) => repl?.setConnection(s),
  onHelloOk: () => {
    repl.setConnection("connected");
  ...
  onEvent: (m) => {
    ...
    if (m.type === "session_started" && (m as { nonce?: string }).nonce === nonce) {
      ...
      repl.setMode((m as { mode?: string }).mode ?? "safelist");
      return;
    }
    ...
    if (m.type === "history") {
      repl.printHistory((m as { items: { role?: string; text?: string }[] }).items);
      return;
    }
    repl.handle(m);
  },
...
repl = startRepl({ conn, clientId, sessionKey: () => sessionKey, agent: args.agent, projectPath: args.projectPath });
